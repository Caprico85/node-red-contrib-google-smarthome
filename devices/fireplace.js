...
module.exports = function(RED) {
    ...
    class FireplaceOnOffNode {
        constructor(config) {
            ...
            if (!this.clientConn) {
                this.error(RED._("fireplace.errors.missing-config"));
                this.status({fill: "red", shape: "dot", text: "Missing config"});
                return;
            } else if (typeof this.clientConn.register !== 'function') {
                this.error(RED._("fireplace.errors.missing-bridge"));
        ...
        registerDevice(client, name, me) {
            ...
            let device = {
                ...
                properties: {
                    ...
                    name: {
                        defaultNames: ["Node-RED On/Off Fireplace"],
                        name: name
                    },
                    ...
                    deviceInfo: {
                        ...
                        model: 'nr-fireplace-onoff-v1',
                        swVersion: '1.0',
                        hwVersion: '1.0'
                    },
                    customData: {
                        "nodeid": client.id,
                        "type": 'fireplace-onoff'
                    }
                }
            };
        ...
        updated(device) {
            ...
            RED.log.debug("FireplaceOnOffNode(updated): states = " + JSON.stringify(states));
         ...
         */
        onInput(msg) {
            RED.log.debug("FireplaceOnOffNode(input)");

            let topicArr = String(msg.topic).split(this.topicDelim);
            let topic    = topicArr[topicArr.length - 1];   // get last part of topic

            RED.log.debug("FireplaceOnOffNode(input): topic = " + topic);

            try {
                if (topic.toUpperCase() === 'ON') {
                    RED.log.debug("FireplaceOnOffNode(input): ON");
                    ...
                    RED.log.debug("FireplaceOnOffNode(input): ONLINE");
                    ...
                    RED.log.debug("FireplaceOnOffNode(input): some other topic");
                    let object = {};

                    if (typeof msg.payload === 'object') {
                        object = msg.payload;
                    } else {
                        RED.log.debug("FireplaceOnOffNode(input): typeof payload = " + typeof msg.payload);
                        return;
                    }
        ...
        onClose(removed, done) {
            if (removed) {
                ...
                this.clientConn.remove(this, 'fireplace-onoff');
            } else {
                // this node is being restarted
                this.clientConn.deregister(this, 'fireplace-onoff');
            }
    ...
    RED.nodes.registerType("google-fireplace-onoff", FireplaceOnOffNode);
}
