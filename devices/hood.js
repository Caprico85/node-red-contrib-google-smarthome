...
module.exports = function(RED) {
    ...
    class HoodOnOffNode {
        constructor(config) {
            ...
            if (!this.clientConn) {
                this.error(RED._("hood.errors.missing-config"));
                this.status({fill: "red", shape: "dot", text: "Missing config"});
                return;
            } else if (typeof this.clientConn.register !== 'function') {
                this.error(RED._("hood.errors.missing-bridge"));
                this.status({fill: "red", shape: "dot", text: "Missing SmartHome"});
                return;
            }

            this.states = this.clientConn.register(this, 'hood-onoff', config.name);
        ...
        registerDevice(client, name) {
            ...
            let device = {
                ...
                properties: {
                    ...
                    name: {
                        defaultNames: ["Node-RED On/Off Hood"],
                        name: name
                    },
        ...
        updated(device) {
            ...
            RED.log.debug("HoodOnOffNode(updated): states = " + JSON.stringify(states));
         ...
         */
        onInput(msg) {
            RED.log.debug("HoodOnOffNode(input)");

            let topicArr = String(msg.topic).split(this.topicDelim);
            let topic    = topicArr[topicArr.length - 1];   // get last part of topic

            RED.log.debug("HoodOnOffNode(input): topic = " + topic);

            try {
                if (topic.toUpperCase() === 'ON') {
                    RED.log.debug("HoodOnOffNode(input): ON");
                    ...
                    RED.log.debug("HoodOnOffNode(input): ONLINE");
                    ...
                    RED.log.debug("HoodOnOffNode(input): some other topic");
                    let object = {};

                    if (typeof msg.payload === 'object') {
                        object = msg.payload;
                    } else {
                        RED.log.debug("HoodOnOffNode(input): typeof payload = " + typeof msg.payload);
                        return;
                    }
        ...
        onClose(removed, done) {
            if (removed) {
                ...
                this.clientConn.remove(this, 'hood-onoff');
            } else {
                // this node is being restarted
                this.clientConn.deregister(this, 'hood-onoff');
            }
