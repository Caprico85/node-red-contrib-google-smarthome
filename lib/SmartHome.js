...
class GoogleSmartHome {
    ...
    getRouter(appInstance) {
        return appInstance.router || appInstance._router;
    }
    //
    //
    //
    ...
    UnregisterUrl(REDapp) {
        const me = this;
        const router = this.getRouter(REDapp);
        ...
        if (router) {
            ...
            let to_remove = [];
            router.stack.forEach(function (route, i) {
                if (route.route && (
            ...
            to_remove.forEach(i => router.stack.splice(i, 1));
    ...
    Start(REDapp, REDserver) {
        ...
        try {
            ...
            if (this._httpPort > 0) {
                ...
                const routerApp = this.getRouter(this.app);
                if (routerApp) {
                    routerApp.stack.forEach((r) => {
            ...
            if (this._httpLocalPort > 0) {
                ...
                const routerLocalApp = this.getRouter(this.localApp);
                if (routerLocalApp) {
                    routerLocalApp.stack.forEach((r) => {
                        if (r.route && r.route.path) {
                            me.debug('SmartHome:Start(): url ' + r.route.path + " registered for " + me.GetRouteType(r));
                        }
                    });
            ...
            if (this._httpPort <= 0) {
                ...
                const routerREDapp = this.getRouter(REDapp);
                if (routerREDapp) {
                    routerREDapp.stack.forEach((r) => {
        ...
        } catch (err) { // This is the catch for the main try block in Start()
            return err;
        }
        // Lines 480 and 481 in the error output seem to be these next two, which are incorrect here.
        // They seem to be remnants of a misapplied patch or copy-paste error.
        // We remove them. The 'return true' should follow the main try-catch.
