...
export default class HttpAuth {
    ...
    httpAuthRegister(httpRoot, appHttp) {
        ...
        /**
         * Handles GET requests to /oauth.
         * 
         * This is called:
         * - As the first step in the login process after the user selected our service from the service list in the app.
         *   We show the login form.
         *   Request contains no relevant headers or body, but query parameters: client_id, redirect_uri, state, response_type.
         * - After an unsuccessful login attempt, when the user is redirected back to the login form.
         *   Request contains no relevant headers or body, but query parameters: client_id, redirect_uri, state, response_type, error.
         *
         * Available parameters:
         * - client_id: The client ID we registered our project with at Google
         * - redirect_uri: The URL to which the user is redirected after the login process
         * - state: A random string for CSRF protection, which is passed back to Google unchanged
         * - response_type: The response type, which must be "code"
         * - error: If the user was redirected back to the login form after an unsuccessful login attempt, this parameter is set to "invalid_user"
         */
        ...
        /**
         * Handles POST requests to /oauth.
         * 
         * This is called:
         * - After the user has entered their credentials in the login form and clicked "Login" or after the user has logged in with Google Sign-In.
         *   On a successful login, we generate an auth code and redirect the user to the redirect_uri. On a failed login, we redirect the user back to the login form.
         *   Request contains no relevant headers or query parameters, but a body with the parameters: username, password, id_token, client_id, redirect_uri, state
         * 
         * Available parameters:
         * - username: The username entered by the user in the login form. Empty if the user logged in with Google Sign-In.
         * - password: The password entered by the user in the login form. Empty if the user logged in with Google Sign-In.
         * - client_id: The client ID we registered our project with at Google
         * - redirect_uri: The URL to which the user is redirected after the login process
         * - state: A random string for CSRF protection, which is passed back to Google unchanged
         * - id_token: The ID token returned by Google after successfully logging in with Google Sign-In. Empty if the user logged in with username and password.
         */
    ...
    _handleUserAuth(req, res, username, password, isValidUser, httpRoot) {
        if (!isValidUser) {
            ...
            this._smarthome._mgmtNode.error('HttpAuth:_handleUserAuth(): invalid user');
        ...
        if (authCode) {
            ...
            let redirectUrl = util.format('%s?client_id=%s&redirect_uri=%s&state=%s&response_type=code',
                this._smarthome.Path_join(httpRoot, 'oauth'), req.body.client_id, encodeURIComponent(req.body.redirect_uri), req.body.state);
            this._smarthome._mgmtNode.error('HttpAuth:_handleUserAuth(): generating authCode failed');
    ...
    _handleAuthCode(req, res, my_uri) {
        ...
        try {
            ...
            this._smarthome._mgmtNode.error('HttpAuth:_handleAuthCode(): ' + err);
            let error_result = {"error": "invalid_grant"};
            res.status(400).send(error_result);
        }
    }
    ...
    _handleRefreshToken(req, res) {
        ...
        try {
            ...
            this._smarthome._mgmtNode.error('HttpAuth:_handleRefreshToken(): ' + err);
            let error_result = {"error": "invalid_grant"};
            res.status(400).send(error_result);
        }
    }
