...
class HttpAuth {
    ...
    handleUserAuth(req, res, username, password, isValidUser, httpRoot) {
        if (!isValidUser) {
            process.nextTick(() => {
                this._smarthome.emitter.emit('/oauth', 'invalid user', username, password);
            });
            ...
            this._smarthome._mgmtNode.error('HttpAuth:handleUserAuth(): invalid user, redirecting back to login page');
            this._smarthome.debug('HttpAuth:handleUserAuth(): redirect URL = ' + redirectUrl);
        ...
        if (authCode) {
            process.nextTick(() => {
                this._smarthome.emitter.emit('/oauth', 'authCode successful', username, password);
            });

            let redirectUrl = util.format('%s?code=%s&state=%s', req.body.redirect_uri, authCode, req.body.state)
            this._smarthome.debug('HttpAuth:handleUserAuth(): authCode successful, redirecting to Google; authCode = ' + authCode);
            this._smarthome.debug('HttpAuth:handleUserAuth(): redirect URL = ' + redirectUrl);
            return res.redirect(redirectUrl);
        } else {
            process.nextTick(() => {
                this._smarthome.emitter.emit('/oauth', 'authCode failed', username, password);
            });

            let redirectUrl = util.format('%s?client_id=%s&redirect_uri=%s&state=%s&response_type=code',
                this._smarthome.Path_join(httpRoot, 'oauth'), req.body.client_id, encodeURIComponent(req.body.redirect_uri), req.body.state);
            this._smarthome._mgmtNode.error('HttpAuth:handleUserAuth(): authCode failed, redirecting back to WOHIN EIGENTLICH???');
            this._smarthome.debug('HttpAuth:handleUserAuth(): redirect URL = ' + redirectUrl);
            return res.redirect(redirectUrl);
        }
    }
