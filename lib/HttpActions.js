...
/** @typedef {import('./SmartHome').GoogleSmartHome} GoogleSmartHome */
...
class HttpActions {
    ...
    reportState(deviceId, states, notifications) {
        ...
        if (!this._smarthome.auth.isUserLoggedIn()) {
            me._smarthome.error('HttpActions:reportState(): skipped reporting states, no user logged in');
            return;
        }
    ...
    requestSync() {
        ...
        if (!this._smarthome.auth.isUserLoggedIn()) {
            me._smarthome.error('HttpActions:requestSync(): skipped requested sync, no user logged in');
            return;
        }
