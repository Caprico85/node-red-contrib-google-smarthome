 ...
 * Copyright (C) 2026 Claudio Chimera, Andreas Schuster and others.
 *
...
describe('Device Node', function () {
    ...
    it('Smart Home should be loaded with correct default params', function (done) {
        ...
        helper.load([GoogleSmartHomeNode, MgmtNode, DeviceNode], flow, function () {
            try {
                const device1 = helper.getNode("device1") as DeviceNode;
                device1.should.have.property('type', 'google-device');
                device1.should.have.property('clientConn');
                const client = helper.getNode(device1.config.client) as MgmtNode;
